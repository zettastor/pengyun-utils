...
remote_workspace="/home/chenlia/network_tune/simple"
...
kill_process() {
    ...
    #ssh chenlia@$ip "ps -ef | grep $flag | grep -v grep | awk '{print \$2}' | xargs -r -n 1 kill -9 "
}
...
rsync -av --delete target/lib chenlia@$client_ip:$remote_workspace
rsync -av --delete target/lib chenlia@$server_ip:$remote_workspace

# start server process
echo "start server"
ssh chenlia@$server_ip "nohup java -cp \"$remote_lib_path/*\" $java_opt $specific_opt $server_class_name $server_listen_ip $server_listen_port > $remote_workspace/server.log 2>&1 < /dev/null &"
sleep 3

# start client process
echo "start client"
ssh chenlia@$client_ip "nohup java -cp \"$remote_lib_path/*\" $java_opt $specific_opt $client_class_name $server_listen_ip $server_listen_port $write_request_length $unit_count_per_request $iodepth $test_write $queue_capacity > $remote_workspace/client.log 2>&1 < /dev/null &"
